...
- Update **both** `tasks/.index.json` and the task markdown file when changing status.
...
6. On completion, move the task file to `tasks/completed/TASK-<NNN>-<slug>.md`.
7. Update `tasks/.index.json` → `"done"`.
